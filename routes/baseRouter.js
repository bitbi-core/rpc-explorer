...
const btcQuotes = require("../app/coins/btbQuotes.js");
...
router.get("/block-height/:blockHeight", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Bitbi Block #${blockHeight.toLocaleString()}`;
			res.locals.metaDesc = "";
		}
...
router.get("/block/:blockHash", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Bitbi Block ${utils.ellipsizeMiddle(res.locals.result.getblock.hash, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/tx/:transactionId", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialTransactions && global.specialTransactions[txid]) {
			...
			res.locals.metaTitle = `Bitbi Transaction ${utils.ellipsizeMiddle(txid, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/address/:address", asyncHandler(async (req, res, next) => {
	try {
		...
		res.locals.metaTitle = `Bitbi Address ${address}`;
...
router.get("/rpc-terminal", function(req, res, next) {
	if (!config.demoSite && !req.authenticated) {
		res.send("RPC Terminal / Browser require authentication. Set an authentication password via the 'BTBEXP_BASIC_AUTH_PASSWORD' environment variable (see .env-sample file for more info).");
