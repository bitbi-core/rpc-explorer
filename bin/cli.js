...
const args = require('meow')(`
	...
	Options
	  ...
	  -C, --coin <coin>			  crypto-coin to enable [default: BTB]
	...
	Examples
	  ...
	  $ btb-rpc-explorer -p 8080 -P 18443 -c ~/.bitcoin/regtest.cookie

	Or using connection URIs
	  $ btb-rpc-explorer -b bitcoin://bob:myPassword@127.0.0.1:18443/
	  $ btb-rpc-explorer -b bitcoin://127.0.0.1:18443/?cookie=$HOME/.bitcoin/regtest/.cookie

	All options may also be specified as environment variables
	  $ BTBEXP_PORT=8080 BTBEXP_BITCOIND_PORT=18443 BTBEXP_BITCOIND_COOKIE=~/.bitcoin/regtest/.cookie btb-rpc-explorer
...
Object.keys(args).filter(k => k.length > 1).forEach(k => {
	if (args[k] === false) {
		debugLog(`Config(arg): BTBEXP_NO_${envify(k)}=true`);

		process.env[`BTBEXP_NO_${envify(k)}`] = true;

	} else {
		debugLog(`Config(arg): BTBEXP_${envify(k)}=${args[k]}`);

		process.env[`BTBEXP_${envify(k)}`] = args[k];
	}
});
