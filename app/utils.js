...
function formatCurrencyAmountWithForcedDecimalPlaces(amount, formatType, forcedDecimalPlaces) {
	...
	if (currencyType.type == "native") {
		...
		if (forcedDecimalPlaces >= 0) {
			...
			// toDP excludes trailing zeroes but doesn't "fix" numbers like 1e-9
			...
			//var baseStr = addThousandsSeparators(dec.toDP(decimalPlaces)); // old version, failed to properly format "1e-9" (left unchanged)
...
function satoshisPerUnitOfLocalCurrency(localCurrency) {
	if (global.exchangeRates != null) {
		...
		var satCurrencyType = global.currencyTypes["wen"];
		...
		return {amt:addThousandsSeparators(exchangedAmt),amtRaw:exchangedAmt, unit:`wen/${localCurrencyType.symbol}`}
	}
...
function getBlockTotalFeesFromCoinbaseTxAndBlockHeight(coinbaseTx, blockHeight) {
	...
	if (blockReward < 1e-9 || blockReward == null) {
		return totalOutput;
