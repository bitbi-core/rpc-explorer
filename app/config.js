...
let baseUrl = (process.env.BTBEXP_BASEURL || "/").trim();
...
const currentCoin = process.env.BTBEXP_COIN || "BTB";
...
const cookieSecret = process.env.BTBEXP_COOKIE_SECRET
 || (rpcCred.password && crypto.createHmac('sha256', JSON.stringify(rpcCred))
                               .update('btb-rpc-explorer-cookie-secret').digest('hex'))
 || "0x000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f";


const electrumServerUriStrings = (process.env.BTBEXP_ELECTRUM_SERVERS || process.env.BTBEXP_ELECTRUMX_SERVERS || "").split(',').filter(Boolean);
...
[
	"BTBEXP_DEMO",
	"BTBEXP_PRIVACY_MODE",
	"BTBEXP_NO_INMEMORY_RPC_CACHE",
	"BTBEXP_RPC_ALLOWALL",
	"BTBEXP_ELECTRUM_TXINDEX",
	"BTBEXP_UI_HIDE_INFO_NOTES",
...
const slowDeviceMode = (process.env.BTBEXP_SLOW_DEVICE_MODE.toLowerCase() == "true");

module.exports = {
	host: process.env.BTBEXP_HOST || "127.0.0.1",
	port: process.env.PORT || process.env.BTBEXP_PORT || 3002,
	...
	displayDefaults: {
		displayCurrency: (process.env.BTBEXP_DISPLAY_CURRENCY || "btb"),
		localCurrency: (process.env.BTBEXP_LOCAL_CURRENCY || "usd"),
		theme: (process.env.BTBEXP_UI_THEME || "dark"),
		timezone: (process.env.BTBEXP_UI_TIMEZONE || "local")
	},

	cookieSecret: cookieSecret,

	privacyMode: (process.env.BTBEXP_PRIVACY_MODE.toLowerCase() == "true"),
	slowDeviceMode: slowDeviceMode,
	demoSite: (process.env.BTBEXP_DEMO.toLowerCase() == "true"),
	queryExchangeRates: (process.env.BTBEXP_NO_RATES.toLowerCase() != "true" && process.env.BTBEXP_PRIVACY_MODE.toLowerCase() != "true"),
	noInmemoryRpcCache: (process.env.BTBEXP_NO_INMEMORY_RPC_CACHE.toLowerCase() == "true"),
	
	rpcConcurrency: (process.env.BTBEXP_RPC_CONCURRENCY || (slowDeviceMode ? 3 : 10)),

	filesystemCacheDir: (process.env.BTBEXP_FILESYSTEM_CACHE_DIR || path.join(process.cwd(),"./cache")),

	noTxIndexSearchDepth: (+process.env.BTBEXP_NOTXINDEX_SEARCH_DEPTH || 3),

	rpcBlacklist:
	  process.env.BTBEXP_RPC_ALLOWALL.toLowerCase() == "true"  ? []
	: process.env.BTBEXP_RPC_BLACKLIST ? process.env.BTBEXP_RPC_BLACKLIST.split(',').filter(Boolean)
	: [
		"addnode",
		"backupwallet",
		"bumpfee",
	...
	addressApi: process.env.BTBEXP_ADDRESS_API,
	electrumTxIndex: process.env.BTBEXP_ELECTRUM_TXINDEX != "false",
	electrumServers: electrumServers,

	redisUrl:process.env.BTBEXP_REDIS_URL,

	site: {
		hideInfoNotes: process.env.BTBEXP_UI_HIDE_INFO_NOTES,
		homepage:{
			recentBlocksCount: parseInt(process.env.BTBEXP_UI_HOME_PAGE_LATEST_BLOCKS_COUNT || (slowDeviceMode ? 5 : 10))
		},
		blockTxPageSize: (slowDeviceMode ? 10 : 20),
		addressTxPageSize: 10,
		txMaxInput: (slowDeviceMode ? 3 : 15),
		browseBlocksPageSize: parseInt(process.env.BTBEXP_UI_BLOCKS_PAGE_BLOCK_COUNT || (slowDeviceMode ? 10 : 25)),
		...
		toolSections: [
			...
			// {name: "Analysis", items: [10, 11, 12, 3]},
	...
	/* 13 */	{name:"Whitepaper Extractor", url:"./bitcoin-whitepaper", desc:"Extract the Bitbi whitepaper from data embedded within the blockchain.", fontawesome:"far fa-file-alt"},
	...
	/* 17 */	{name:"Quotes", url:"./quotes", desc:"Curated list of Bitbi-related quotes.", fontawesome:"fas fa-comment-dots"},
