...
const configPaths = [
	path.join(os.homedir(), ".config", "btb-rpc-explorer.env"),
	path.join("/etc", "btb-rpc-explorer", ".env"),
...
if (process.env.BTBEXP_BASIC_AUTH_PASSWORD) {
	// basic http authentication
	expressApp.use(auth(process.env.BTBEXP_BASIC_AUTH_PASSWORD));

} else if (process.env.BTBEXP_SSO_TOKEN_FILE) {
	// sso authentication
	expressApp.use(sso(process.env.BTBEXP_SSO_TOKEN_FILE, process.env.BTBEXP_SSO_LOGIN_REDIRECT_URL));
}
...
async function getSourcecodeProjectMetadata() {
	var options = {
		url: "https://api.github.com/repos/janoside/btb-rpc-explorer",
		headers: {
			'User-Agent': 'request'
		}
	};
...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
	...
	if (match) {
		...
		debugErrorLog(`Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of Bitbi Core?`);
	}
...
expressApp.continueStartup = function() {
	...
	if (config.addressApi) {
		...
		if (!supportedAddressApis.includes(config.addressApi)) {
			utils.logError("32907ghsd0ge", `Unrecognized value for BTBEXP_ADDRESS_API: '${config.addressApi}'. Valid options are: ${supportedAddressApis}`);
		}
		...
		if (config.addressApi == "electrum" || config.addressApi == "electrumx") {
			if (config.electrumServers && config.electrumServers.length > 0) {
				...
				utils.logError("327hs0gde", "You must set the 'BTBEXP_ELECTRUM_SERVERS' environment variable when BTBEXP_ADDRESS_API=electrum.");
			}
		}
	}
